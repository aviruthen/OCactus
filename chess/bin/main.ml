...
let rec move_game mv bs =
  ...
  let m =
    if State.get_fifty bs >= 101 then
  ...
  let m =
    if
      List.length
        (List.filter
           (fun b -> State.cmp_boards b bs)
           (State.get_prev_boards bs))
      = 3
    then
      let _ = State.print_board bs in
      let _ =
  ...
  | valid_move ->
      ...
      if nbs = bs then (
        ...
        Stdlib.print_string ("Enter a valid move for " ^ get_turn bs ^ ": ");
        move_game (String.trim (read_line ())) bs)
      else if State.get_fifty nbs >= 101 then (
        ...
        List.length
          (List.filter
             (fun b -> State.cmp_boards b nbs)
             (State.get_prev_boards nbs))
        = 3
      then (
        let _ = State.print_board nbs in
        ANSITerminal.print_string [ ANSITerminal.green ]
        ...
        List.length (State.all_legal_moves (State.pseudolegal_moves nbs)) = 0
      then
