...
let parse inp =
  if inp |> String.lowercase_ascii |> String.trim = "quit" then
    { command = ("quit", "game") }
  else if inp |> String.lowercase_ascii |> String.trim = "u" then
    { command = ("u", "u") }
  else
    let sq_list = String.split_on_char ' ' (String.trim inp) in
    if List.length sq_list <> 2 then raise MalformedInput
    else
      match sq_list with
      | [ a; b ] ->
          if String.length a <> 2 || String.length b <> 2 then
            ...
            then raise MalformedInput
              ...
              else { command = (sq1, sq2) }
      | _ -> raise MalformedInput

let get_command cmd =
  let a, b = cmd.command in
  a ^ " " ^ b
